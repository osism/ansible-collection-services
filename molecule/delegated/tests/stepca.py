...
def test_stepca_api_health(host):
    stepca_host = get_variable(host, "stepca_host")
    stepca_port = get_variable(host, "stepca_port")

    # Test health endpoint
    health_url = f"http://{stepca_host}:{stepca_port}/health"
    result = host.run(f"curl -s -o /dev/null -w '%{{http_code}}' {health_url}")
    assert result.rc == 0
    assert result.stdout.strip() == "200"

    # Verify health response contains "ok"
    result = host.run(f"curl -s {health_url}")
    assert result.rc == 0
