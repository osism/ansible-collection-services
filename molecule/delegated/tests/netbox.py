...
def test_config_dirs(host):
    directories = [
        ...
        get_variable(host, "netbox_configuration_directory_netbox"),
    ...
    for d in directories:
        ...
        assert d.mode == 0o750
        assert d.user == get_variable(host, "netbox_operator_user")
        assert d.group == get_variable(host, "netbox_operator_group")


def test_postgres_config(host):
    config_dir = get_variable(host, "netbox_configuration_directory_netbox")
    ...
    assert postgres_env.user == get_variable(host, "netbox_operator_user")
    assert postgres_env.group == get_variable(host, "netbox_operator_group")
    assert (
    ...
    assert postgres_pw.user == get_variable(host, "netbox_operator_user")
    assert postgres_pw.group == get_variable(host, "netbox_operator_group")
    assert (
    ...
    assert entrypoint.user == get_variable(host, "netbox_operator_user")
    assert entrypoint.group == get_variable(host, "netbox_operator_group")
    ...
    assert db_script.user == get_variable(host, "netbox_operator_user")
    assert db_script.group == get_variable(host, "netbox_operator_group")
...
def test_netbox_config(host):
    ...
    for d in directories:
        ...
        assert f.mode == 0o755
        assert f.user == get_variable(host, "netbox_operator_user")
        assert f.group == get_variable(host, "netbox_operator_group")
    ...
    assert netbox_env.user == get_variable(host, "netbox_operator_user")
    assert netbox_env.group == get_variable(host, "netbox_operator_group")
    ...
    assert config_py.user == get_variable(host, "netbox_operator_user")
    assert config_py.group == get_variable(host, "netbox_operator_group")
    assert (
    ...
    assert nginx_unit.user == get_variable(host, "netbox_operator_user")
    assert nginx_unit.group == get_variable(host, "netbox_operator_group")
    ...
    for script in startup_scripts:
        ...
        assert s.mode == 0o644
        assert s.user == get_variable(host, "netbox_operator_user")
        assert s.group == get_variable(host, "netbox_operator_group")
...
def test_dockernetwork(host):
    ...
    with host.sudo("root"):
        ...
        assert get_variable(host, "netbox_traefik_external_network_name") in stdout
