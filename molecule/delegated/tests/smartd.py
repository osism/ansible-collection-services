...
def test_smartmontools_package_installed(host):
    ...
    assert smartd_package.is_installed, f"Package {smartd_package_name} should be installed"
...
def test_smartd_log_directory_exists(host):
    ...
    assert log_directory.user == "root", "/var/log/smartd directory should be owned by root"
    assert log_directory.group == "root", "/var/log/smartd directory should be in the root group"
    assert log_directory.mode == 0o755, "/var/log/smartd directory should have 755 permissions"
...
def test_smartmontools_config_file(host):
    ...
    assert config_file.user == "root", "smartmontools configuration file should be owned by root"
    assert config_file.group == "root", "smartmontools configuration file should be in the root group"
    assert config_file.mode == 0o644, "smartmontools configuration file should have 644 permissions"
